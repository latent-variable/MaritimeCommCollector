...
#            _first_uri_xpath = "//a[contains(@href,'/archives/download/')]"
    ...
    def _get_entry_datetimes(self, times):
        ...
        # Handle any string that is not a valid time
        valid_times = []
        for each in times:
            try:
                valid_times.append(_dt.datetime.strptime(each, '%I:%M %p').time())
            except ValueError:
                pass
                # print(f"Invalid time format: {each}")
                # You can replace the invalid time with a default value if needed
                # valid_times.append(_dt.time(0, 0))  # Default value: 00:00

        if len(valid_times) < 2:
            # Handle the case where there are not enough valid times
            print("Not enough valid times")
            return None  # or some default value

        hhmm_start, hhmm_end = valid_times
